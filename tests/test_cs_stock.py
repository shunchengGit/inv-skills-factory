...
def test_snapshot_a_stock():
    """A 股快照返回完整数据。"""
    result = run_script("cs-stock", "cs_stock_info.py", "snapshot", VALID_A_STOCK, "--output", "json", timeout=60)
...
def test_snapshot_etf():
    """ETF 快照返回完整数据。"""
    result = run_script("cs-stock", "cs_stock_info.py", "snapshot", "513010", "--output", "json", timeout=60)
...
def test_snapshot_hk_stock():
    """港股快照返回数据（可能需要代理）。"""
    result = run_script("cs-stock", "cs_stock_info.py", "snapshot", VALID_HK_STOCK, "--output", "json", timeout=60)
...
def test_daily_a_stock():
    """A 股日线返回数据。"""
    result = run_script("cs-stock", "cs_stock_info.py", "daily", VALID_A_STOCK, "--output", "json", timeout=60)
...
def test_financial_a_stock():
    """A 股财务摘要返回数据。"""
    result = run_script("cs-stock", "cs_stock_info.py", "financial", VALID_A_STOCK, "--output", "json", timeout=60)
    if result.returncode != 0:
...
def test_invalid_code():
    """无效代码返回合理错误。"""
    result = run_script("cs-stock", "cs_stock_info.py", "snapshot", "999999", "--output", "json", timeout=60)
