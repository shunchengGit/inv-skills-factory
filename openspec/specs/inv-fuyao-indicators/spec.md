...
inv-fuyao-indicators 的 `fuyao_indicators.py` SHALL 改为调用 `_shared/indicators/` 框架的 `build_snapshot()` 和 `render_text_snapshot()`，而非内联实现。
...
`inv-fuyao-indicators/_meta.json` 的 `dependencies` SHALL 反映实际依赖关系。
