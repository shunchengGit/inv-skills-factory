...
**完成标志**：
- `/qarp` ✅ 完成：输出含五档结论 + 三道闸门结果 + 估值纪律判定 + 买入必答 6 问 + 组合检查 + 机会成本对比的完整报告，给出明确买入/观察/不买结论
- `/qarp_screen` ✅ 完成：输出三道闸门逐项通过/不通过 + 一句话结论（"值得深入研究，建议 /qarp" 或 "不通过：{原因}"）
- `/qarp_check` ✅ 完成：输出估值检查 + 打脸条件核查 + 组合检查 + 明确持有/加仓/减仓/卖出结论及触发条件
...
**卖方趋势校验**：ROE/FCF/毛利率趋势须与知识库研报的卖方判断交叉验证。若卖方趋势判断与财报相悖，按 `research-cross-validation.md` 优先级规则处理（财报事实 > 卖方预测）；若卖方多份研报一致指出趋势恶化而财报尚未体现，标注"卖方预警"作为风险信号。
