...
**阶段一（并行）**：A. `valuation_snapshot.py <代码> --output json` + B. 按 `inv-knowledge-curator` 深度挖掘协议 L2 标准深度搜索知识库（见其 `references/deep-mining-protocol.md`）——多角度搜索（标的/估值/风险/行业 ≥3 个角度）→ 跟随条目 `## 关联` 链 ≥1 层 → 标签导航收尾。搜索结果须按 `references/research-cross-validation.md` 的映射表结构化输出：共识观点（行业/TAM/竞争）→ 第一道闸门；财务趋势→ 第二道闸门；盈利预测区间→ 估值纪律 3.3；核心风险/隐含假设→ 买入必答。若知识库无记录，按 4.5 格式输出知识缺口。
