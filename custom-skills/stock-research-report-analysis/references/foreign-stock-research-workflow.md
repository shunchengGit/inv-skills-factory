...
1. **cs-stock**：先尝试 `snapshot` → 若失败，依次尝试 `financial` + `profile`（不同端点限流粒度不同）
2. **cs-stock financials**：当 CLI snapshot 全失败时，`cs_stock_info.py financials` 可获取利润表/资产负债表/现金流量表，从三表中可提取核心财务数据
...
当本地研报时效性不足或缺少关键事件时，通过 cs-crawl 搜索补充：
