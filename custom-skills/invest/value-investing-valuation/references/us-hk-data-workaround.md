...
> 适用场景：`cs-stock` 或 `valuation_snapshot.py` 对美股/港股返回大量 `data_gaps`（Yahoo Finance 限流/代理不稳定）
...
### A. cs-stock financials 子命令（优先）

当 `valuation_snapshot.py` 返回大量 data_gaps 时，先用 cs-stock 的 financials 子命令获取财务三表：

```bash
uv run {valuationDir}/../cs-stock/scripts/cs_stock_info.py financials 0700.HK --output json
```

返回 `income_stmt`、`balance_sheet`、`cash_flow` 三个表，字段映射与提取方式与 yfinance 相同（cs-stock 内部封装了 yfinance financials 端点）。
...
**若 cs-stock financials 也失败**（info SSL + financials 同时不可用），才降级到 Step B（cs-crawl）或手动计算。

### B. 通过 cs-crawl 抓取 Yahoo Finance Analysis 页面

当所有 yfinance 端点均不可用，或需要分析师一致预期时，调用 cs-crawl 技能：

```bash
# 1. 用 cs-crawl 抓取 Yahoo Analysis 页面
...
加载并遵循 `stock-research-report-analysis/SKILL.md`，用 `research_pdf.py list --code {TICKER}` 定位近半年研报，然后 `extract` 获取：
...
- **SSL / Connection reset**: `info()` 和 `recommendations()` 端点因 SSL 握手失败时，优先降级到 `financials` 端点 + cs-crawl 抓取（见 Step B）。
