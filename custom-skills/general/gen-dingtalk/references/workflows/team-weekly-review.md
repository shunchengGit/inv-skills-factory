...
如果 workspaceId 已知，可跳过搜索直接进入 Step 2。
...
```
**1 业务迭代**
**点点开黑：** 3.3.9版本预计5月25日提审。已完成8项需求：消费限额合规提醒、关注功能优化、广场/照片墙视频内容标注、产品体验优化、电竞培训机构合作支持、FM厅支持小游戏、帖子评论支持删除、播放音视频降低厅内音量。进行中4项：首页厅列表背景展示框、羊村之巅新厅模板、RTC双通道支持、王者buff赛。未开始：车队内优化。
**点点狼人：** iOS 3.3.4版本已于5月15日提审，3.3.5版本计划26号提审，已完成私聊搜索页、Unity退出误报过滤、队长申请流程优化，待完成我的页无网兜底处理（运营反馈：接口失败导致空页面需客户端兜底）。Android 3.4.0版本已于5月15日提审。双端同步点点平台功能。
**兔小铲：** 双端完成分享逻辑适配、帖子删除评论、金铲铲小厅铲铲会、厅战力榜；iOS额外完成照片墙5.0、人脸识别进厅逻辑、群组。1.5.5版本已于5月20日发布。
...
**架构升级**
iOS Socket优化： 已在队长包验证，暂未反馈问题，整体改造完成进入稳定性验证阶段。
双端RN探索： 兔小铲已上线练习bundle，RN SDK进度30%；Android RN容器化完成；下拉刷新改为原生组件实现。
**研发运维**
核心指标研发运维优化（高优）： 统计时间由30+分钟缩短到5分钟左右，APM自定义看板已配置，崩溃和网络告警均已配置。
```

格式规则：
1. **业务迭代**：每个产品线一段，产品名加粗后冒号起头，版本计划→已完成→进行中→未开始依次紧凑叙述，用句号或分号分隔
2. **技术专项**：按技术项目大类（性能优化/架构升级/研发运维）分组，每个子项单独一行，子项目名后冒号+空格，紧跟量化数据和里程碑，不加项目符号
3. **标题加粗**：一级标题（"1 业务迭代"、"2 技术专项"）、二级分类标题（"性能优化"、"架构升级"、"研发运维"）和产品线名称均使用加粗
4. **技术专项过滤**：仅保留有重大进展的项目（有量化数据或关键里程碑），跳过"暂无进展"、"/"、"自测中无数据"等项目
5. **纯文本输出**：不要 HTML、不要 Markdown 表格、不要代码块包裹、不要生成文件

## 注意事项

1. **文件夹范围**：只看"周会周报"文件夹下的周报，不要搜索整个知识库
2. **内容范围**：只要业务迭代 + 技术专项，不要质量指标、产品体验、个人工作情况、TODO 等章节
3. **技术专项过滤**：无重大进展不提，有量化数据才保留
4. **周报时效**：优先最新期，但需检查是否为空模板
5. **子文档关联**：主周报内嵌了技术指标周报和产品体验周报的链接，但这些子文档主要用于补充技术指标数据，核心业务迭代信息在主周报的"3 业务相关"表格中
6. **中文编码**：所有 `dws` 命令必须通过 Python 调用，设置 UTF-8 环境变量，不要直接在 Bash 中执行
