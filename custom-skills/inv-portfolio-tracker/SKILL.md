...
1. **结构固定且顺序固定**：`[ 组合概览 ]` → 持仓表 → `[ 纪律检查 ]` → `[ 关键关注 ]`，缺一不可。
2. **列固定**：`| 标的 | 代码 | 股数 | 价格 | 市值(万) | 仓位 | PE | 52w位 | 备注 |`（`REPORT_COLUMNS` 是唯一列来源）。
3. **终端等宽对齐**：表格由 `render_table()` 按 `unicodedata.east_asian_width` 计算宽字符后对齐，直接展示脚本 stdout，不要重新排版。
4. **纯文本**：不使用 `###` 标题、`**` 加粗或超链接；段落标题统一为 `[ 标题 ]`。
5. **组合概览前置**：总资产、现金（人民币/港币/美元明细及折 CNY）、汇率与行情截至时间。
6. **价格带币种标记**：`HK$` / `$` / `¥`；市值统一为万 CNY。
7. **脚本只取 stdout**：脚本日志走 stderr，不要把日志行混入展示内容。

**网络不可用或解析失败时**：回退为读取 PORTFOLIO.md 中的最新「当前持仓」数据，但仍需套用同一结构（列固定、等宽对齐、三个 section 齐全），并注明数据为文件内快照时间。不得自行发明其他布局。
...
[ 纪律检查 ]
- 单只上限 (<= 40%): 腾讯控股 占比 35.8% (达标)
- 现金比例 (>= 2%): 当前 1.6% (未达标，现金偏低)
- 行业集中度 (<= 55%): 软件/互联网平台 54.0% (达标)

[ 关键关注 ]
- 现金水位: 现金占比 1.6% 低于 2% 下限，优先补充现金至 5-10% 区间
```
