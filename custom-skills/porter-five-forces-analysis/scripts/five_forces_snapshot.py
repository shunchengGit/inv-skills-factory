...
def _call_cs_stock(*args: str) -> dict:
    """调用 cs-stock CLI 并返回解析后的 JSON dict。代理通过环境变量自动管理。"""
    from pathlib import Path
...
def _parse_news_items(raw_news: list[dict[str, Any]] | None, limit: int = 5) -> list[NewsItem]:
    """将 cs-stock 返回的 news 列表转为 NewsItem。"""
    items: list[NewsItem] = []
...
def build_snapshot(symbol: str, forced_market: str) -> Snapshot:
    ...
    if is_a_share(symbol):
        ...
        if snap and not snap.get("error"):
            ...
            notes.append(f"cs-stock snapshot 调用失败: {err}")
        ...
        if desc_data and not desc_data.get("error"):
            data_sources.append("cs-stock(description)")
        ...
        if ann_data and not ann_data.get("error"):
            ...
            notes.append(f"cs-stock announcements 调用失败: {err}")
        ...
        if rel_data and not rel_data.get("error"):
            ...
            notes.append(f"cs-stock relations 调用失败: {err}")
        ...
        if recent_news:
            data_sources.append("cs-stock(news)")
    ...
    # A 股：从 cs-stock 中文 key 补充缺失的财务指标
