...
| **gen-dingtalk-daily-log** | 在个人钉钉文档中更新每日工作日志 |
| **gen-dingtalk-group-report** | 按月分析钉钉群聊消息并生成报告 |
| **gen-dingtalk-weekly-mail** | 从钉钉周报生成邮件草稿存入企业微信邮箱 |
| **gen-dingtalk-weekly-review** | 从钉钉知识库读取并总结团队周报 |
...
gen-dingtalk（钉钉底层能力）────────────────────────┐
  ↑                                                │
  ├── gen-dingtalk-daily-log（个人工作日志）        │
  ├── gen-dingtalk-group-report（群聊分析报告）    │
  ├── gen-dingtalk-weekly-mail（周报邮件）        │
  └── gen-dingtalk-weekly-review（周报总结）        │
```
