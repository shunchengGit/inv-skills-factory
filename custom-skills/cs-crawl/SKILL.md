...
name: cs-crawl
description: >-
  Web 搜索（SearXNG）+ 全页抓取（Firecrawl）+ 浏览器抓取的工具箱。
...
| `/cs_crawl_scrape <url>` | 全页抓取 → Markdown |
...
| browser_navigate | 有 | 慢 | SPA/JS渲染页面 |
...
**browser 走代理**：浏览器工具自身不走系统代理，对境外网站需确认可达性。
...
| 问题 | 解决 |
|------|------|
| `Connection refused` | 本地 Firecrawl 未启动，用 browser_navigate 替代 |
| `Connection reset` / SSL 错误 | 代理不稳定，检查代理是否正常 |
| 境外网站超时 | 检查代理是否启动，设置 HTTP_PROXY/HTTPS_PROXY |
| 返回空内容 | 页面可能是 SPA，用 browser 代替 |
| 中文搜索结果差 | 换英文关键词重试 |
